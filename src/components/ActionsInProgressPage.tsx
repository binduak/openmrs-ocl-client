...
const ActionsInProgressPage: React.FC<Props> = ({
  ...
  let inProgressListOne = inProgressList?.length > 0 ? updateLocalStorageArray({
    name:"notification",
    key: "inProgressList",
  ...
  let loadingListOne = loadingList?.length > 0 ? updateLocalStorageArray({
      name:"notification",
      key: "loadingList",
  ...
  let erroredListOne = erroredList?.length > 0 ? updateLocalStorageArray({
    name:"notification",
    key: "erroredList",
  ...
  let successListOne = successList?.length > 0 ? updateLocalStorageArray({
    name:"notification",
    key: "successList",
  ...
  const inProgressItems = loadingListOne
    .map((loading: boolean | undefined, index: number) =>
      typeof loading == "boolean" && loading ? inProgressListOne[index] : null
    )
    .filter((item: any) => item)
    .reverse() as string[];

  const successfullItems = loadingListOne
    .map((loading: boolean | undefined, index: number) =>
      typeof loading == "boolean" && !loading && !erroredListOne[index]
        ? {
            result: successListOne[index]?.payload || "Successful",
            progress: inProgressListOne[index]
          }
        : null
    )
  ...
  const erroredItems = loadingListOne
    .map((loading: boolean | undefined, index: number) =>
      typeof loading == "boolean" && !loading && erroredListOne[index]
        ? {
            error: erroredListOne[index],
            progress: inProgressListOne[index]
          }
        : null
    )
